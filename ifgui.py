from PyQt5.QtWidgets import QApplication, QMessageBox, QDesktopWidget, QMainWindow, QPushButton, QLabel, QWidget, \
    QInputDialog, QLineEdit, QFileDialog, QVBoxLayout
        ...
        if self.inputWindow is None:
            try:
                ...
                start, okPressed = QInputDialog. \
                    getText(self, "Get coordinates",
                            "ENTER STARTING POINT (TOP LEFT):\n in format x,y\n for example 120,85", QLineEdit.Normal,
                            "", )
                end, okPressed = QInputDialog. \
                    getText(self, "Get coordinates",
                            "ENTER ENDING POINT (BOTTOM RIGHT):\n in format x,y\n for example 120,85",
                            QLineEdit.Normal, "")
    ...
    def rotate(self):
        if self.inputWindowOfRotation is None:
            try:
                if (len(self.loadedImagePath) == 0):
                    raise FileNotFoundError
                rotationDegree, okPressed = QInputDialog.getText(self, "Rotation", "Enter Rotation Degree",
                                                                 QLineEdit.Normal, "", )
        ...
        if self.inputWindow is None:
            try:
                ...
                newRGB, okPressed = QInputDialog. \
                    getText(self, "Change Color Balance", (
                    "enter your increase or decrease value in format r,g,b\n For example:112,144,96 or 112,-50,-32"),
                            QLineEdit.Normal, "", )
                image = cv2.imread(self.loadedImagePath)
